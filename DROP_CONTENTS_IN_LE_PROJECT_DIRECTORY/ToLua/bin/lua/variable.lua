	...
	if class and static==nil then
		...
		if string.find(self.mod, 'tolua_inherits') then
			local push_func = get_push_function(self.type)
				...
				local name = self:inmodule("module")
				if name == nil then name = self:inmodule("namespace") end
				if name ~= nil then
					name = name .. "::" .. self:getvalue(class,static,prop_get)
				else
					name = self:getvalue(class,static,prop_get)
				end
			
				local t,ct = isbasic(self.type)
				if t then
					output('  tolua_push'..t..'(tolua_S,(',ct,')'..name..');')
				else
					local push_func = get_push_function(self.type)
						t = self.type
						if self.ptr == '&' or self.ptr == '' then
							output('  ',push_func,'(tolua_S,(void*)&'..name..',"',t,'");')
						else
							output('  ',push_func,'(tolua_S,(void*)'..name..',"',t,'");')
						end
					end
